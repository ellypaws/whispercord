// @ts-nocheck
export function bootDesktopController() {
  if (window.__vtDesktopControllerBooted) return;
  window.__vtDesktopControllerBooted = true;
  ...
  if (window.pywebview && window.pywebview.api) { void boot(); } else { window.addEventListener("pywebviewready", boot, { once: true }); }
}
